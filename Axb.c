...
datos.
**/
int sendvector(int who_send, int, int*);
...
int distributereceive(int f, int c, int *m, int *x, int size) {
	...
	*/
	...
	/*
	Se crea la variable fila la cual contiene los valores de la fila que van a
	ser enviados a los procesos esclavos creados, este envio se realiza de
	manera similar a una cola y cuando ya se han ocupado todos se vuelve a
	enviar tareas desde el primer proceso esclavo
	*/
	for (int i = 0; i < f; i++)
	{
		int fila[f];
		...
		MPI_Send(fila, MAXCOL, MPI_INT, (i%(size-1)) + 1, TAGTAREA, MPI_COMM_WORLD);
	}

	/*
	Se reciben los valores de multiplicacion de cada proceso esclavo en igual
	orden en que se envio, garantizando que los valores correspondan a las
	posiciones de la matriz esperada
	*/
	for (int i = 0; i < f; ++i)
	{
		int value;
		MPI_Recv(&value, 1, MPI_INT, (i%(size-1)) + 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Stat);
		x[i] = value;
	}

	/*
	Se le indican a todos los procesos que deben detener su ejecucion
	*/
	for (int i = 1; i < size; ++i)
	{
		int nothing = 5;
		MPI_Send(&nothing, 1, MPI_INT, i, TAGPARAR, MPI_COMM_WORLD);
	}
	return 0;
}
...
int receive(int rank) {
	...
	*/
	...
	/*
	Primero se recibe el vector que va a multiplicar la matriz, posterior a
	esto se empiezan a recibir las nuevas instrucciones si se debe realizar
	una tarea se realiza el proceso de multiplicar fila por columna y se envia
	el valor obtenido al maestro, caso contrario termina su ejecucion
	*/
	int fila[MAXCOL];
	int vector[MAXCOL];
	MPI_Recv(vector, MAXCOL, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &Stat);
	while (Stat.MPI_TAG != TAGPARAR)
	{
		...
		if (Stat.MPI_TAG != TAGPARAR)
		{
			for (int i = 0; i < MAXCOL; ++i)
			{
				int val1 = fila[i];
				int val2 = vector[i];
				returnValue = (val1 * val2) + returnValue;
			}
			MPI_Send(&returnValue, 1, MPI_INT, 0, TAGTAREA, MPI_COMM_WORLD);
		}
	}
	return 0;
}
